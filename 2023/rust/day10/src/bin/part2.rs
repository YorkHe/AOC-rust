...
fn part2(input: &str) -> u32 {
    ...
    while !queue.is_empty() {
        ...
        if map[x][y] == 'S' {
            ...
            else if x > 0 && (map[x - 1][y] == 'F' || map[x - 1][y] == '7' || map[x - 1][y] == '|') {
                queue.push(((x - 1, y), steps + 1));
            }
            else if y + 1 < map[0].len()
                && (map[x][y + 1] == 'J' || map[x][y + 1] == '7' || map[x][y + 1] == '-')
            {
                queue.push(((x, y + 1), steps + 1));
            }
            else if y > 0 && (map[x][y - 1] == 'L' || map[x][y - 1] == 'F' || map[x][y - 1] == '-') {
                queue.push(((x, y - 1), steps + 1));
            }
        } else {
            match map[x][y] {
...
mod tests {
    #[test]
    fn test_part2() {
        let input = include_str!("../../../../input/day10/example2.txt");
