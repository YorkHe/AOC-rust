...
fn part1(input: &str) -> Result<u32> {
    ...
    let mut part_set: HashSet<&Part> = HashSet::new();

    for symbol in symbols {
        for offset in -1..=1 {
            let x: i32 = symbol.0 as i32;
            let y: i32 = symbol.1 as i32 + offset;
    ...
    let sum = part_set.iter().fold(0, |acc, part| acc + part.part_num);
